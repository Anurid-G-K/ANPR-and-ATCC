...
if st.session_state.step == 1:
    ...
    with col_analyze:
        if uploaded_file is not None:
            if st.button("Analyze Video for ANPR and ATCC", key="analyze_button", use_container_width=True):
...
# STEP 2: Analysis Page
    ...
    if 'analytics_data' not in st.session_state:
        st.session_state.analytics_data = {
            ...
            'vehicle_types_history': {'Car': [], 'Motorcycle': [], 'Bus': [], 'Truck': []},
        }
...
# Create placeholders - video on top, analytics below
frame_placeholder = st.empty()
progress_bar = st.progress(0)

st.markdown("<div style='margin-top: 3.5rem;'></div>", unsafe_allow_html=True)  # Space after progress bar

results_table_placeholder = st.empty()

st.markdown("<div style='margin-top: 2rem;'></div>", unsafe_allow_html=True)  # Space after table

download_button_placeholder = st.empty()

st.markdown("<div style='margin-top: 3.5rem;'></div>", unsafe_allow_html=True)  # Space after download button

vehicle_counter_placeholder = st.empty()

st.markdown("<div style='margin-top: 3.5rem;'></div>", unsafe_allow_html=True)  # Space before graphs

# Analytics charts placeholders (AFTER vehicle counter)
...
def update_analytics_data(detections, vehicle_type_map):
    """Track analytics data over time from live vehicle counter"""
    type_counts = {t: 0 for t in vehicle_type_map.values()}
    total = 0
    ...
    for vtype in vehicle_type_map.values():
        st.session_state.analytics_data['vehicle_types'][vtype] = type_counts[vtype]
    
    # Update history
    current_time = datetime.now().strftime("%H:%M:%S")
    st.session_state.analytics_data['timestamps'].append(current_time)
    st.session_state.analytics_data['vehicle_counts'].append(total)
    
    for vtype in vehicle_type_map.values():
        st.session_state.analytics_data['vehicle_types_history'][vtype].append(type_counts[vtype])
    
    # Keep only last 50 data points to avoid memory issues
    if len(st.session_state.analytics_data['timestamps']) > 50:
        st.session_state.analytics_data['timestamps'] = st.session_state.analytics_data['timestamps'][-50:]
        st.session_state.analytics_data['vehicle_counts'] = st.session_state.analytics_data['vehicle_counts'][-50:]
        for vtype in vehicle_type_map.values():
            st.session_state.analytics_data['vehicle_types_history'][vtype] = st.session_state.analytics_data['vehicle_types_history'][vtype][-50:]

def create_vehicle_distribution_chart():
    """Create pie chart for vehicle type distribution"""
    data = st.session_state.analytics_data
    
    # Get current counts
    current_counts = data['vehicle_types']
    
    # Filter out zero values
    labels = [k for k, v in current_counts.items() if v > 0]
    values = [v for v in current_counts.values() if v > 0]
    
    if not values:
        # Return empty chart with message
        fig = go.Figure()
        fig.add_annotation(
            text="No vehicles detected yet",
    ...
    fig.update_layout(
        title=dict(text='<b>Vehicle Type Distribution</b>', font=dict(size=18, color='white')),
        showlegend=True,
        height=424,
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)',
...
def create_detection_timeline_chart():
    """Create line chart for detection timeline"""
    data = st.session_state.analytics_data
    ...
    # Add line for each vehicle type
    colors = {'Car': '#E94B3C', 'Motorcycle': "#E1FF00", 'Bus': "#008CFF", 'Truck': "#00FF11"}
    
    for vtype, color in colors.items():
        fig.add_trace(go.Scatter(
            x=data['timestamps'],
            y=data['vehicle_types_history'][vtype],
            mode='lines+markers',
            name=vtype,
            line=dict(color=color, width=2),
            marker=dict(size=4),
            hovertemplate=f'<b>{vtype}</b><br>Time: %{{x}}<br>Count: %{{y}}<extra></extra>'
        ))
    
    fig.update_layout(
        title=dict(text='<b>Detection Timeline</b>', font=dict(size=18, color='white')),
        xaxis=dict(
            title='Time',
            showgrid=True,
        ...
        margin=dict(l=20, r=20, t=75, b=100)
    )
    
    return fig

def create_detection_rate_gauge():
    """Create gauge chart for detection rate"""
    data = st.session_state.analytics_data
    
    if len(data['vehicle_counts']) < 1:
        rate = 0
    else:
        # Get current frame count
        rate = data['vehicle_counts'][-1]
    
    fig = go.Figure(go.Indicator(
        mode="gauge+number",
        value=rate,
        domain={'x': [0, 1], 'y': [0, 1]},
        title={'text': "<b>Traffic Density Meter</b>", 'font': {'size': 18, 'color': 'white'}},
        ...
        gauge={
            ...
            'bordercolor': "white",
            'steps': [
                {'range': [0, 5], 'color': 'rgba(78, 205, 196, 0.3)'},
                {'range': [5, 10], 'color': 'rgba(255, 160, 122, 0.3)'},
                {'range': [10, 20], 'color': 'rgba(255, 107, 107, 0.3)'}
            ],
            'threshold': {
...
def flush_results_csv_and_ui(best_vehicle_log, csv_path, frame_key):
    ...
    if rows:
        ...
        download_button_placeholder.download_button(
            ...
            mime='text/csv',
            key=f"download_results_{frame_key}"
        )
...
while ret:
    ...
    # Update analytics data (every 3 frames to reduce overhead)
    if frame_nmr % 3 == 0:
        update_analytics_data(detections, vehicle_type_map)
    ...
    for license_plate in license_plates.boxes.data.tolist():
        ...
        if car_id != -1:
            ...
            if license_plate_text is not None:
                ...
                (text_width, text_height), _ = cv2.getTextSize(license_plate_text, cv2.FONT_HERSHEY_SIMPLEX, 1.575, 3)
                ...
                box_width = text_width + (2 * padding_horizontal)
                box_height = text_height + (2 * padding_vertical)

                # Position box above license plate
                box_x1 = int((x1 + x2 - box_width) / 2)
                box_y1 = int(y1) - box_height - 5
                ...
                # Calculate centered text position (baseline position for putText)
                text_x_centered = box_x1 + padding_horizontal
                text_y_centered = box_y2 - padding_vertical

                cv2.putText(frame, license_plate_text, (text_x_centered, text_y_centered), cv2.FONT_HERSHEY_SIMPLEX, 1.575, (0, 0, 0), 3, cv2.LINE_AA)
                ...
                cv2.rectangle(frame, (lbl_x - 6, lbl_y - lbl_h - 6), (lbl_x + lbl_w + 6, lbl_y + 6), (255, 255, 255), -1)
    ...
    now = datetime.now()
    cv2.putText(frame, f'Frame: {frame_nmr}', (15, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255), 2)
    cv2.putText(frame, f'Date: {now.strftime("%Y-%m-%d")}', (15, 90), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255), 2)
    cv2.putText(frame, f'Time: {now.strftime("%H:%M:%S")}', (15, 130), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255), 2)
    ...
    f'<div class="centered-video"><img src="data:image/png;base64,{img_str}" /></div>',
    unsafe_allow_html=True
)
    ...
    # Update charts (every 3 frames to reduce overhead)
    if frame_nmr % 3 == 0:
        # Create layout structure once
        if frame_nmr == 3:  # Only create layout on first update
            with pie_chart_placeholder.container():
                top_col1, top_col2 = st.columns([1, 1])
                pie_col = top_col1.empty()
                gauge_col = top_col2.empty()
            
            timeline_col = timeline_chart_placeholder.empty()
            
            # Store in session state for reuse
            st.session_state.pie_col = pie_col
            st.session_state.gauge_col = gauge_col
            st.session_state.timeline_col = timeline_col
        
        # Update charts in existing placeholders
        if hasattr(st.session_state, 'pie_col'):
            pie_chart = create_vehicle_distribution_chart()
            st.session_state.pie_col.plotly_chart(pie_chart, use_container_width=True, key=f"live_pie_chart_{frame_nmr}", config={'displayModeBar': False, 'displaylogo': False})
            
            gauge_chart = create_detection_rate_gauge()
            st.session_state.gauge_col.plotly_chart(gauge_chart, use_container_width=True, key=f"live_gauge_chart_{frame_nmr}", config={'displayModeBar': False, 'displaylogo': False})
            
            timeline_chart = create_detection_timeline_chart()
            st.session_state.timeline_col.plotly_chart(timeline_chart, use_container_width=True, key=f"live_timeline_chart_{frame_nmr}", config={'displayModeBar': False, 'displaylogo': False})
...
while ret:
    ...
    for row_indx in range(len(df_frame)):
        try:
            ...
            if plate_text != '0' and plate_text != 'N/A':
                (text_width, text_height), _ = cv2.getTextSize(plate_text, cv2.FONT_HERSHEY_SIMPLEX, 1.575, 3)
                ...
                cv2.putText(frame, plate_text, (text_x_centered, text_y_centered), cv2.FONT_HERSHEY_SIMPLEX, 1.575, (0, 0, 0), 3, cv2.LINE_AA)
    ...
    now = datetime.now()
    cv2.putText(frame, f'Frame: {frame_nmr}', (15, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.1, (255, 255, 255), 2)
    cv2.putText(frame, f'Date: {now.strftime("%Y-%m-%d")}', (15, 90), cv2.FONT_HERSHEY_SIMPLEX, 1.1, (255, 255, 255), 2)
    cv2.putText(frame, f'Time: {now.strftime("%H:%M:%S")}', (15, 130), cv2.FONT_HERSHEY_SIMPLEX, 1.1, (255, 255, 255), 2)
